...
Processor.prototype.parseNext = function () {
	...
	if (this.options.commentEscape && text.startsWith("//" + this.options.symbol))
		return;
	...
	if (text.indexOf(this.options.symbol) !== 0)
	...
	if (i != -1) {
		name = text.substr(this.options.symbol.length, i - 1);
		text = text.substr(i + 1);
	} else {
		name = text.substr(this.options.symbol.length);
	}
